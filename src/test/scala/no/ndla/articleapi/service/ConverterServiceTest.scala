...
class ConverterServiceTest extends UnitSuite with TestEnvironment {
  ...
  test("That the document is wrapped in an article tag") {
    ...
    val node = NodeToConvert(List(contentTitle), List(contentNode), copyright, List(tag), Seq(pageTitle), Seq(visualElement), Seq(), "fagstoff", 0, 1)
  ...
  test("That the ingress is not added to the content") {
    ...
    val node = NodeToConvert(List(contentTitle), List(contentNodeBokmal, contentNodeNynorsk), copyright, List(tag), Seq(pageTitle), Seq(visualElement), Seq(), "fagstoff", 0, 1)
