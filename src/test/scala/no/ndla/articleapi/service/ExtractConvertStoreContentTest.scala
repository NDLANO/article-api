...
import no.ndla.articleapi.integration.{LicenseDefinition, LanguageContent, MigrationSubjectMeta}
...
class ExtractConvertStoreContentTest extends UnitSuite with TestEnvironment {
  ...
  val sampleContent = LanguageContent(nodeId, nodeId, contentString, "metadescription", Some("en"))
