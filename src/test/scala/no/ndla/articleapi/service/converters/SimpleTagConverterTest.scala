...
class SimpleTagConverterTest extends UnitSuite {
  ...
  test("That divs with class 'paragraph' are replaced with section") {
    ...
    val Success((result, _)) = SimpleTagConverter.convert(sampleLanguageContent, ImportStatus.empty)
  ...
  test("That divs with class 'ndla_table' is converted to table") {
    ...
    val result = SimpleTagConverter.convert(sampleLanguageContent, ImportStatus.empty)
