...
class ContentBrowserConverterTest extends UnitSuite with TestEnvironment {
  ...
  override def beforeEach = {
    contentBrowserConverter.reset()
  }

  test("That content-browser strings are replaced") {
    val initialContent = Content(s"<article><p>$sampleContentString</p></article>", Some("en"))
    ...
    val result = contentBrowserConverter.convert(initialContent)

    result.content.replace("\n", "") should equal (expectedResult)
    contentBrowserConverter.requiredLibraries.length should equal (0)
  }

  test("That content-browser strings of type h5p_content are converted correctly") {
    val nodeId = "1234"
    val initialContent = Content(s"<article>$sampleContentString</article>", Some("en"))
    ...
    result.content.replace("\n", "") should equal (expectedResult)
    contentBrowserConverter.requiredLibraries.length should equal (1)
  }
