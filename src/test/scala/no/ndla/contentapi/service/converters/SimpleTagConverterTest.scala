...
import no.ndla.contentapi.model.Content
import no.ndla.learningpathapi.UnitSuite

class SimpleTagConverterTest extends UnitSuite {
  override def beforeEach = {
    SimpleTagConverter.reset()
  }

  test("That divs with class 'paragraph' are replaced with section") {
    val initialContent = Content("<article><h1>heading</h1><div class='paragraph'>I know words, I have the best words.</div></article>", Some("en"))
    val expectedResult = "<article> <h1>heading</h1> <section>  I know words, I have the best words. </section></article>"
    val result = SimpleTagConverter.convert(initialContent)

    result.content.replace("\n", "") should equal (expectedResult)
    SimpleTagConverter.requiredLibraries.length should equal (0)
  }

  test("That divs with class 'full' are removed") {
    val initialContent = Content("<article><div class='full'><h1>heading</h1>A small loan of a million dollars</div></article>", Some("en"))
  ...
  test("That children of pre tags are wrapped in code tags") {
    val initialContent = Content("<article><h1>heading</h1><pre>I know words, I have the best words.</pre></article>", Some("en"))
  ...
  test("That divs with class 'quote' are replaced with a blockquote tag") {
    val initialContent = Content("""<article><h1>heading</h1><div class="quote">I know words, I have the best words.</div></article>""", Some("en"))
  ...
  test("That divs with class 'right' are replaced with a aside tag") {
    val initialContent = Content("""<article><div class="right">I know words, I have the best words.</div></article>""", Some("en"))
  ...
  test("That divs with class 'hide' converted to details-summary tags") {
    val initialContent = Content("""<article><div class="hide">Eksempel: <a href="#" class="read-more">les mer</a>
      <div class="details">
  ...
  test("That body is converted to article") {
    val initialContent = Content("""<body><div class="right">I know words, I have the best words.</div></body>""", Some("en"))
    val expectedResult = "<article> <aside>  I know words, I have the best words. </aside></article>"
    val result = SimpleTagConverter.convert(initialContent)
