...
class V27__AddConceptIdsToArticle extends BaseJavaMigration {
  ...
  private[migration] def convertArticleUpdate(document: String): String = {
    ...
    val toMergeWith = JObject("conceptIds" -> JArray(List.empty))

    val mergedArticle = oldArticle.merge(toMergeWith)
