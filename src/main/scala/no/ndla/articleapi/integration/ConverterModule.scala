...
case class LanguageContent(nid: String, tnid: String, content: String, language: Option[String],
  ...
  def asArticleIntroduction: Option[ArticleIntroduction] = ingress.map(x => ArticleIntroduction(x.content.getOrElse(""), language))
}

case class LanguageIngress(content: Option[String])
